...
import { SITE_OPTIONS, extractGenderFromIdCard, getDefaultPasswordFromIdCard, validateOneInchPhoto } from '@/lib/workflow'
...
export default function ExamStaffManage() {
  ...
  const [filingArea, setFilingArea] = useState('全部')
  ...
  const filingAreas = ['全部', ...Array.from(new Set(SITE_OPTIONS.map(item => item.filingArea)))]
  const filtered = useMemo(() => staff.filter(item => {
    const bySearch = !search || [item.name, item.idCard, item.workUnit, item.site].join(' ').includes(search)
    const byArea = filingArea === '全部' || item.filingArea === filingArea
    return bySearch && byArea
  }), [filingArea, search, staff])

  const saveStaff = (next: Omit<ExamStaff, 'id' | 'assignStatus' | 'status'>) => {
    if (editing) {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between border-b border-gray-200 bg-white px-4 py-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button onClick={() => toast.success('导入完成，请核对照片与身份证')} className="bg-[#1A56DB] hover:bg-[#1748B5]">
      ...
      <div className="grid min-h-[620px] grid-cols-[180px_minmax(0,1fr)] gap-6">
        <aside className="rounded-lg border border-gray-200 bg-white p-4">
          ...
          <div className="mt-3 space-y-1">
            {filingAreas.map(place => (
              <button
                key={place}
                onClick={() => setFilingArea(place)}
                className={`w-full rounded px-3 py-2 text-left text-sm hover:bg-gray-50 ${filingArea === place ? 'bg-blue-50 text-blue-700' : 'text-gray-600'}`}
              >
                {place}
              </button>
            ))}
                ...
                {filtered.map((item, index) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-flex rounded-full px-2 py-0.5 text-xs ${item.photoValidated ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
                        {item.photoValidated ? '已校验' : '未通过'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-flex rounded-full px-2 py-0.5 text-xs ${item.status === '有效' ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600'}`}>{item.status}</span>
                    </td>
                    <td className="px-4 py-3 text-center">
...
function StaffDialog({
  ...
  open: boolean
  title: string
  initial: Partial<ExamStaff> | null
  onClose: () => void
  onSave: (staff: Omit<ExamStaff, 'id' | 'assignStatus' | 'status'>) => void
}) {
