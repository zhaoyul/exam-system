...
export default function TrainingPage({ type = '督导培训' }: { type?: TrainingType }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Dialog open={dialog === 'register'} onOpenChange={() => setDialog(null)}><DialogContent><DialogHeader><DialogTitle>人员报名 - {active?.name}</DialogTitle></DialogHeader><div className="space-y-3 text-sm"><Field label="证件号码" name="idCard" defaultValue="440301198001011234" /><Field label="姓名" name="name" defaultValue="陈建国" /><div className="rounded-md bg-[#F9FAFB] p-3">点击查询可自动识别历史人员信息，也可手工录入。</div><div className="flex justify-end gap-2"><Button variant="outline" onClick={() => toast.success('已查询历史报名信息')}><Search className="mr-2 h-4 w-4" />查询</Button><Button onClick={() => { setItems(prev => prev.map(item => item.id === active?.id ? { ...item, registered: item.registered + 1 } : item)); setDialog(null); toast.success('培训人员已录入') }}><UserPlus className="mr-2 h-4 w-4" />录入信息</Button></div></div></DialogContent></Dialog>
