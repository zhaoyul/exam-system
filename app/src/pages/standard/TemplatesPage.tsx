...
const firstLevelTypes = ['仅标题', '基本要求', '工作要求', '权重表']
const secondLevelTypes = ['仅标题', '文本输入', '富文本输入', '文化程度选择', '等级选择', '基本要求', '工作要求', '权重表']
...
export default function TemplatesPage() {
  ...
  const [customChildren, setCustomChildren] = useState<Record<string, any[]>>({})
  ...
  const [newItem, setNewItem] = useState({ title: '', type: '仅标题', parentId: '', level: '一级' })
  ...
  const visibleItems = [...backendTemplateItems, ...customItems]
  const doAddItem = () => {
    if (!newItem.title) return
    const item = { id: `custom-${Date.now()}`, title: newItem.title, type: newItem.type, level: newItem.level === '一级' ? 1 : 2 }
    if (newItem.parentId) {
      setCustomChildren(prev => ({ ...prev, [newItem.parentId]: [...(prev[newItem.parentId] || []), item] }))
    } else {
      setCustomItems(prev => [...prev, item])
    }
    setNewItem({ title: '', type: '仅标题', parentId: '', level: '一级' })
  ...
  return (
    <div>
      ...
      {!selectedTpl ? (
        ...
        <div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <div className="flex items-center justify-between mb-3"><div><h3 className="text-sm font-semibold text-gray-900">模板结构</h3><div className="mt-1 text-xs text-gray-500">一级类型：{firstLevelTypes.join(' / ')}；二级类型：{secondLevelTypes.join(' / ')}</div></div><button onClick={() => setShowAddItem(true)} className="h-8 px-3 bg-[#1A56DB] text-white rounded-md text-xs flex items-center gap-1 hover:bg-[#1748B5]"><Plus className="w-3.5 h-3.5" /> 增加新项</button></div>
            {visibleItems.map(item => (
              <div key={item.id}>
                <div className="flex items-center gap-2 py-2 px-3 rounded-md hover:bg-gray-50">
                  {((item.children && item.children.length > 0) || customChildren[item.id]?.length > 0) && (<button onClick={() => toggle(item.id)} className="text-gray-400">{expanded[item.id] ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}</button>)}{!item.children && !customChildren[item.id]?.length && <div className="w-4" />}
                ...
                {((item.children && item.children.length > 0) || customChildren[item.id]?.length > 0) && expanded[item.id] && (<div className="ml-6">{[...(item.children || []), ...(customChildren[item.id] || [])].map(child => (
                  <div key={child.id}>
                    ...
                    {child.children && expanded[child.id] && (<div className="ml-6">{child.children.map((sub: any) => (<div key={sub.id} className="flex items-center gap-2 py-1.5 px-3 rounded-md hover:bg-gray-50"><div className="w-4" /><span className="text-sm text-gray-600">{sub.title}</span><span className="px-2 py-0.5 bg-gray-100 text-gray-600 rounded text-xs">{sub.type}</span></div>))}</div>)}
                  </div>
                ))}</div>)}
              </div>
            ))}
      ...
      {showAddItem && (<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center" onClick={() => setShowAddItem(false)}><div className="bg-white rounded-lg shadow-xl w-[520px]" onClick={e => e.stopPropagation()}><div className="flex items-center justify-between px-6 py-4 border-b border-gray-200"><h3 className="text-base font-semibold text-gray-900">增加模板内容项</h3><button onClick={() => setShowAddItem(false)} className="text-gray-400 hover:text-gray-600"><X className="w-5 h-5" /></button></div><div className="p-6 grid grid-cols-2 gap-4"><div className="col-span-2"><label className="block text-sm text-gray-700 mb-1">项目名称 *</label><input value={newItem.title} onChange={e => setNewItem({ ...newItem, title: e.target.value })} className="w-full h-9 px-3 border border-gray-200 rounded-md text-sm" placeholder="如：职业定义、工作内容、技能要求" /></div><div><label className="block text-sm text-gray-700 mb-1">层级</label><select value={newItem.level} onChange={e => setNewItem({ ...newItem, level: e.target.value, type: e.target.value === '一级' ? '仅标题' : '文本输入' })} className="w-full h-9 px-3 border border-gray-200 rounded-md text-sm"><option>一级</option><option>二级</option></select></div><div><label className="block text-sm text-gray-700 mb-1">项目类型</label><select value={newItem.type} onChange={e => setNewItem({ ...newItem, type: e.target.value })} className="w-full h-9 px-3 border border-gray-200 rounded-md text-sm">{(newItem.level === '一级' ? firstLevelTypes : secondLevelTypes).map(type => <option key={type}>{type}</option>)}</select></div>{newItem.level === '二级' && <div className="col-span-2"><label className="block text-sm text-gray-700 mb-1">父级项目</label><select value={newItem.parentId} onChange={e => setNewItem({ ...newItem, parentId: e.target.value })} className="w-full h-9 px-3 border border-gray-200 rounded-md text-sm"><option value="">作为独立二级项</option>{visibleItems.map(item => <option key={item.id} value={item.id}>{item.title}</option>)}</select></div>}</div><div className="flex justify-end gap-2 px-6 py-4 border-t border-gray-200 bg-gray-50 rounded-b-lg"><button onClick={() => setShowAddItem(false)} className="h-9 px-4 border border-gray-200 rounded-md text-sm hover:bg-gray-100">取消</button><button onClick={doAddItem} disabled={!newItem.title} className="h-9 px-4 bg-[#1A56DB] text-white rounded-md text-sm disabled:opacity-40">保存</button></div></div></div>)}
    </div>
  )
}
