  ...
  [["/dashboard"
   ...
   ["/score"
    ...
    ["/scoring"
     ...
     ["/batch"
      {:post {:summary "批量录入成绩"
              :handler (partial score-controller/batch-create-scores ctx)}}]
     ["/:plan-id/lock"
      {:post {:summary "锁定计划成绩"
              :handler (partial score-controller/lock-plan-scores ctx)}}]
     ["/:plan-id/unlock"
      {:post {:summary "解锁计划成绩"
              :handler (partial score-controller/unlock-plan-scores ctx)}}]
     ["/:id"
      {:get {:summary "查看成绩详情"
             :handler (partial score-controller/get-score ctx)}
       :put {:summary "修改成绩（写入audit_log）"
             :handler (partial score-controller/update-score ctx)}
       :delete {:summary "删除成绩"
                :handler (partial score-controller/delete-score ctx)}}]
     [""
      {:get {:summary "成绩列表"
   ...
   ["/certificate"
    ...
    ["/batches"
     [""
      {:get {:summary "批次列表" :handler (partial certificate/list-batches ctx)}
       :post {:summary "创建批次" :handler (partial certificate/create-batch ctx)}}]
     ["/:id"
      {:get {:summary "批次详情" :handler (partial certificate/get-batch ctx)}
       :put {:summary "更新批次" :handler (partial certificate/update-batch ctx)}}]
     ["/:id/generate"
