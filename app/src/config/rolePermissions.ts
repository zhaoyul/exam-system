...
export const ROLES: RoleInfo[] = [
  { key: 'group_admin', label: '集团管理员', description: '集团端系统管理，拥有全部权限', portalPath: '/dashboard' },
  { key: 'branch_admin', label: '机构管理员', description: '分支机构管理，负责本单位认定工作', portalPath: '/branch/portal' },
  { key: 'expert', label: '评价专家', description: '参与命题、阅卷、考评工作', portalPath: '/expert/workbench' },
...
export const ROLE_MENU_ACCESS: Record<UserRole, string[] | undefined> = {
  ...
  branch_admin: [
    ...
    '/cert/exec/plans',
    '/cert/exec/register',
    '/cert/exec/room-arrange',
    '/cert/exec/exam-arrange',
    '/cert/exec/score-manage',
    '/cert/exec/publicity',
    '/cert/exec/cert-manage',
    '/cert/exec/print',
    '/cert/historical',
    '/cert/special',
    '/cert/fee-settings',
  ...
  expert: [
    ...
    '/question/paper-req',
    '/question/skill-subject',
    '/question/skill-module',
...
export const ROLE_PORTAL_CARDS: Record<UserRole, PortalCard[]> = {
  ...
  branch_admin: [
    { title: '本月报名考生', value: 56, unit: '人', trend: '+5', icon: 'UserPlus', color: 'blue', path: '/cert/exec/register' },
    { title: '待编排考场', value: 3, unit: '场', trend: '需关注', icon: 'LayoutGrid', color: 'amber', path: '/cert/exec/room-arrange' },
  ...
  candidate: [
    ...
    { title: '已获证书', value: 1, unit: '张', trend: '', icon: 'Award', color: 'emerald', path: '/personal/cert-query' },
    { title: '成绩查询', value: '查看', unit: '', trend: '', icon: 'BarChart', color: 'purple', path: '/personal/score-query' },
  ],
}
