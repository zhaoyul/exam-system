...
export default function MarkingLeadPage() {
  ...
  const [tasks, setTasks] = useBackendResourceState<MarkingTask>('/grading', initialTasks)
  const expertRows = useBackendResourceList('/supervision/expert-info', expertOptions.map(name => ({ id: name, name })))
  const backendExpertOptions = expertRows.map(item => item.name)
  ...
  return (
    <div className="space-y-4">
      ...
      <Dialog open={dialog === 'expert'} onOpenChange={() => setDialog(null)}><DialogContent><DialogHeader><DialogTitle>添加阅卷专家</DialogTitle></DialogHeader><form onSubmit={saveExperts} className="space-y-3 text-sm"><div className="grid grid-cols-2 gap-2">{backendExpertOptions.map(name => <label key={name} className="rounded-md border border-gray-100 px-3 py-2"><input type="checkbox" name="experts" value={name} defaultChecked={active?.experts.includes(name)} className="mr-2" />{name}</label>)}</div><label className="block"><span className="font-medium text-gray-700">阅卷组长</span><select name="leader" defaultValue={active?.leader || backendExpertOptions[0]} className="mt-1 h-9 w-full rounded-md border border-gray-200 px-2">{backendExpertOptions.map(name => <option key={name}>{name}</option>)}</select></label><div className="flex justify-end"><Button type="submit">保存</Button></div></form></DialogContent></Dialog>
